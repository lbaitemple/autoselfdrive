...
def getRange(data,angle):
    carAngle = angle+269
    if carAngle > 359:
        carAngle = int(carAngle-359)
    while math.isinf(data.ranges[int(carAngle)]):
        lef=int(carAngle)-1
        rig=int(carAngle)+1
        if lef<0:
            lef=lef+360
        if rig>359:
            rig=rig-360
        while math.isinf(data.ranges[lef]):
            lef=lef-1
            if lef<0:
               lef=lef+360
        while math.isinf(data.ranges[rig]):
            rig=rig+1
            if rig>359:
                rig=rig-360
        return (data.ranges[lef]+data.ranges[rig])/2
